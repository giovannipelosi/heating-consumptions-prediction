% function used to train multiple networks -> 
% variables :
%   output_number {string} = name of the file (output_<output_number> ,
%       figure_<output_number>)
%   data_type range = 1 for "heating" , 2 for "elec"
%   season range = 1 to 3 
%   dataset range = 1 to 28
%   nets_layers {cell of layers} ---> TO BE ADDED (now there are 14 fixed
%       set of layers -> 7 for "small" and 7 for "all"
%   nets_options {cell of options}

%   return -> the execution time is seconds and save the files
% for each "layers" and "options" create 4 outputs (4 datasets in input :
% small , small_deseason, all, all_deseason

function train_net(output_number, data_type, season, dataset_index, nets_options)
